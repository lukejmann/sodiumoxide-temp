...
fn make_libsodium(target: &str, source_dir: &Path, install_dir: &Path) -> PathBuf {
    ...
    // Always run `git submodule update --init --recursive` before checking for the `configure` script
    Command::new("git")
    .args(&["submodule", "update", "--init", "--recursive"])
    .status()
    .expect("Failed to update submodule. Please ensure that git is installed and that you have internet access.");

    let configure_path = source_dir.join("configure");
    if !configure_path.exists() {
        panic!("Failed to find configure script 1! Did you clone the submodule at `libsodium-sys/libsodium`?");
    }
    let configure_path_canonical = fs::canonicalize(&configure_path).expect("Failed to find configure script 2! Did you clone the submodule at `libsodium-sys/libsodium`?");

    let mut configure_cmd = Command::new(configure_path_canonical);
